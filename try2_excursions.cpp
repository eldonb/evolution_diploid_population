...
/* *********
the optimum is defined as all genotypes are homozygous 1/1; 
i.e. fixed for genotype 1 at all chromosomes 
...
private : 
  ...
  /* hence 4N alleles  at each locus */
  static const size_t popsize = 5E5;
  ...
  static constexpr double alphaone = 3.0;
  ...
  /* excursion records excursion to fixation of a single locus */
 ...
 void assigngenome( const unsigned pone, const  unsigned ptwo )
{
  ...
  /* now optimum trait value is $z0 = 2$; homoz $1/1$ at all loci */
  z.push_back( rexp( exp( ( -s )*pow( 2.0  -  ( static_cast<double>(sumg) / dchroms ), 2. ) ) ) ) ;
}
  ...
  bool lossoftype( )
{
  ...
  // const unsigned k = 4*(self->popsize);
  ...
  return( std::any_of( counttypes.begin(), counttypes.end(), []( size_t i){ return i < 1; } ) ) ;
}
...
size_t count_types( const double nth )
{
  ...
  /* returns number of copies of type 1 at locus 0 */
  ...
  for( const std::vector<size_t>& v: juveniles){
	...
	/* stop when all chroms are genotype 2 equiv  1/1 */
	hlocus += ( g < 2 ? 0 : 1);
	/* count type at locus to check if lost types; counting among surviving juveniles; if lost genotype among surviving juveniles then automatically have lost it in the population */
	counttypes[ell] += g; 
	/* count 1/1  genotypes at locus */
	counts[ell] += (g < 2 ? 0 : 1) ;
	gsl_matrix_uint_set( pop, ell, i, g);
	ell++;}
      /* record the number of  1 type among surviving juveniles for a single locus */
      
      /* counting number of individuals with all genotypes 1/1 */
 ...
 size_t  updatepop( )
{
  ...
  */
  ...
  /* record number of type 1 copies among surviving juveniles for excursion  */
  number_type_one_copies =  count_types( nthelmnt() );

  return number_type_one_copies ; 
}
...
{
  /* ********* compiling 
c++ -Wall -m64 -O3 -mtune=corei7 -march=native try2_excursions.cpp -lm -lgsl -lgslcblas
************* */
  init();
  ...
  initG();
  excursion.clear();
  /* starting with one copy of type 1 */
  ntypeone = 1;
  do{
    excursion.push_back( ntypeone); 
    ++timi; 
   ntypeone =  updatepop();}
  while(  ( !lossoftype() ) &&  ( (goodtypes) <  (popsize)*2 ) );
  /* goodtypes is number of diploid individuals with all good types */
  std::cout << (goodtypes < (popsize) * 2 ? 0 : 1) << ' ' << timi << '\n';
  /* print excursion to file if  fixation */
  if( goodtypes < ( (popsize) * 2) ){}
  else{
    /* fixation, so print excursion to file */
    std::string excursion_file_name = "excursions_N" + std::to_string(popsize) + "_a3_s" + std::to_string(s) + ".txt"  ;
    std::ofstream excursion_file( "excursions_N5E5_a3_s10.txt", std::ofstream::out | std::ofstream::app );
    for( size_t k = 0; k < timi; ++k){
      excursion_file << excursion[k] << ' ' ;}
    /* add total number of alleles */
    excursion_file << 4*popsize << '\n'; 
    /* new line  excursion_file << '\n';  */
